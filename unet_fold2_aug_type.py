...
skf = KFold(n_splits=5, shuffle= True, random_state=42)                 # kfold split
kfolds = []
...
cp = ModelCheckpoint("regression_model/fold5_20200517_Augfull50_poly_elu_hn_d5_41({epoch:02d})_val_maeOverFscore_keras({val_maeOverFscore_keras:.5f}).hdf5", 
                     monitor='val_maeOverFscore_keras', verbose=1, save_best_only=True, mode='min', period=3)
model = unet()

model.fit(X_train, y_train,
          validation_data = [X_valid, y_valid], 
           epochs=epochs+10, batch_size=batch_size,#steps_per_epoch=len(X_train)//batch_size+1,
          callbacks = [cp]
         )
save_model_architecture('regression_model/fold5_20200517_Augfull50_poly_elu_hn_d5_41', model)